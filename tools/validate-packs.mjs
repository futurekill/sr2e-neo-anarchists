// Pre-flight validator for the Neo-Anarchists' Guide packs. No Foundry: checks
// JSON, required keys, _key form, dup _ids. Run: node tools/validate-packs.mjs
import { readdirSync, readFileSync, statSync } from "node:fs";
import { join } from "node:path";
const ROOT = "packs-src";
const COLLECTION = {
  "na-weapons": "items", "na-armor": "items", "na-gear": "items",
  "na-lifestyles": "items", "na-vehicles": "actors"
};
let problems = 0;
...
for (const pack of packs.sort()) {
  ...
  if (!collection) { note(`${pack}: unknown pack (add to COLLECTION)`); continue; }
